...
export const NavbarTop = () => {
  ...
  useEffect(() => {
    // Close settings if click is outside of the settings dropdown
    const handleClickOutside = (event) => {
      if (
        settingsRef.current &&
        !settingsRef.current.contains(event.target) &&
        svgIconRef.current &&
        !svgIconRef.current.contains(event.target)
      ) {
    ...
    <div className="h-5 w-[98%] fixed z-50 ">
      <header className="w-full flex justify-center items-center mt-5 ">
        <ul className="ml-60 z-100 flex justify-between p-1 px-5 border-solid border-[1px] w-3/5 bg-black bg-opacity-[10%] border-[#5E445F]  rounded-full  text-[12px]">
          ...
          <Link to={"/explore"} className="text-white z-50">
            <li
              className={`${
                selected === 4
                  ? "bg-black bg-opacity-5 border-[#5E445F] border-[0.5px]"
        ...
        <div>
          ...
          {!showSettings && <div className="absolute top-0 ml-56 mt-12 hidden peer-hover:block">
